    ...
    void step(bool enableMovement) {
        ...
        for (unsigned y = 0; y < height; ++y) {
            for (unsigned x = 0; x < width; ++x) {
                ...
                const unsigned px = target % width;
                const unsigned py = target / width;
                ...
                for (int k = 0; k < 8; ++k) {
                    ...
                    const unsigned xx = wrap(static_cast<int>(px) + dx, width);
                    const unsigned yy = wrap(static_cast<int>(py) + dy, height);
                    const unsigned ni = idx(xx, yy);
...
int main(int argc, char** argv) {
    ...
    while (window.isOpen()) {
        ...
        if (state == State::Menu) {
            ...
            startLabel.setFillColor(neon);
            window.draw(startLabel);

            if (hasFont) {
                sf::Text title(font);
                title.setString("EVOLVING LIFE");
                title.setCharacterSize(88);
                title.setFillColor(sf::Color{neon.r, neon.g, neon.b, 255});
#if defined(SFML_VERSION_MAJOR) && (SFML_VERSION_MAJOR >= 3)
                const auto b = title.getLocalBounds();
                title.setOrigin(sf::Vector2f{b.position.x + b.size.x * 0.5f, b.position.y + b.size.y * 0.5f});
#else
                const auto b = title.getLocalBounds();
                title.setOrigin(sf::Vector2f{b.left + b.width * 0.5f, b.top + b.height * 0.5f});
#endif
                title.setPosition(sf::Vector2f{kWindowWidth * 0.5f, kWindowHeight * 0.28f});
                window.draw(title);

                sf::Text hint(font);
                hint.setString("SPACE: pause   R: reset   C: clear   N: nutrients   T: trails   M: movement\nLeft click: seed cross (Shift = species 2)   Right click: nutrient patch");
                hint.setCharacterSize(26);
                hint.setFillColor(sf::Color{200, 220, 220, 220});
#if defined(SFML_VERSION_MAJOR) && (SFML_VERSION_MAJOR >= 3)
                const auto hb = hint.getLocalBounds();
                hint.setOrigin(sf::Vector2f{hb.position.x + hb.size.x * 0.5f, hb.position.y + hb.size.y * 0.5f});
#else
                const auto hb = hint.getLocalBounds();
                hint.setOrigin(sf::Vector2f{hb.left + hb.width * 0.5f, hb.top + hb.height * 0.5f});
#endif
                hint.setPosition(sf::Vector2f{kWindowWidth * 0.5f, kWindowHeight * 0.78f});
                window.draw(hint);
            }

        } else {
            const std::size_t n = static_cast<std::size_t>(kGridWidth) * kGridHeight;

            for (std::size_t i = 0; i < n; ++i) {
                trail[i] *= kTrailDecay;
                if (sim.alive[i]) {
                    trail[i] = std::max(trail[i], 0.55f + 0.45f * clamp01(sim.energy[i] * 0.85f));
                }
            }
            ...
            for (unsigned y = 0; y < kGridHeight; ++y) {
                for (unsigned x = 0; x < kGridWidth; ++x) {
                    ...
                    const float t = showTrails ? clamp01(trail[i]) : (sim.alive[i] ? 1.f : 0.f);

                    sf::Color base = sf::Color::Black;

                    if (showNutrients) {
                        const float nrm = clamp01(sim.nutrient[i] / kMaxNutrient);
                        base = lerp(sf::Color{0, 0, 0, 255}, sf::Color{20, 40, 55, 255}, nrm);
                    }

                    sf::Color cell = base;

                    if (t > 0.f) {
                        const sf::Color s0{0u, 255u, 190u, 255u};
                        const sf::Color s1{255u, 95u, 210u, 255u};
                        const sf::Color aliveCol = sim.species[i] ? s1 : s0;
                        cell = lerp(base, aliveCol, t);
                    }
            ...
            if (hasFont) {
                ...
                oss << "Alive S0: " << sim.aliveBySpecies[0] << "   Alive S1: " << sim.aliveBySpecies[1] << "\n";
                auto avg = [](float sum, std::size_t count) { return (count > 0) ? (sum / static_cast<float>(count)) : 0.f; };
                oss << std::fixed << std::setprecision(2);
                oss << "Avg HarvestMul S0: " << avg(sim.harvestMulSum[0], sim.aliveBySpecies[0])
                    << "   S1: " << avg(sim.harvestMulSum[1], sim.aliveBySpecies[1]) << "\n";
                oss << "Avg UpkeepMul S0: " << avg(sim.upkeepMulSum[0], sim.aliveBySpecies[0])
                    << "   S1: " << avg(sim.upkeepMulSum[1], sim.aliveBySpecies[1]) << "\n";
                oss << "Avg BirthMul   S0: " << avg(sim.birthMulSum[0], sim.aliveBySpecies[0])
                    << "   S1: " << avg(sim.birthMulSum[1], sim.aliveBySpecies[1]) << "\n";
                oss << "Avg AgeMul     S0: " << avg(sim.ageMulSum[0], sim.aliveBySpecies[0])
                    << "   S1: " << avg(sim.ageMulSum[1], sim.aliveBySpecies[1]) << "\n";
                oss << "Paused: " << (paused ? "yes" : "no")
                    << "   Nutrients: " << (showNutrients ? "on" : "off")
                    << "   Trails: " << (showTrails ? "on" : "off")
                    << "   Movement: " << (enableMovement ? "on" : "off");

                hud.setCharacterSize(22);
                hud.setFillColor(sf::Color{230, 245, 245, 230});
                hud.setString(oss.str());
                hud.setPosition(sf::Vector2f{18.f, 14.f});

                sf::RectangleShape panel;
#if defined(SFML_VERSION_MAJOR) && (SFML_VERSION_MAJOR >= 3)
                const auto hb = hud.getLocalBounds();
                panel.setSize(sf::Vector2f{hb.size.x + 18.f, hb.size.y + 18.f});
#else
                const auto hb = hud.getLocalBounds();
                panel.setSize(sf::Vector2f{hb.width + 18.f, hb.height + 18.f});
#endif
                panel.setPosition(sf::Vector2f{12.f, 10.f});
